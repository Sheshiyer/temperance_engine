...
for SH in /bin/sh /opt/homebrew/bin/bash; do
  ...
  ck "[$SH] model(validation)"   "command-code:inclusionai/ling-3.0-flash-free" "$(model_of "$SH" 'audit the code')"
  ck "[$SH] model(fast)"         "command-code:inclusionai/ling-3.0-flash-free" "$(model_of "$SH" 'fix typo in header')"
  ck "[$SH] model(creative)"     "command-code:poolside/laguna-s-2.1-free"   "$(model_of "$SH" 'brainstorm ideas')"
  ck "[$SH] model(balanced)"     "command-code:poolside/laguna-s-2.1-free"   "$(model_of "$SH" 'do the thing')"
