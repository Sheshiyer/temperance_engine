...
for SH in /bin/sh /opt/homebrew/bin/bash; do
  ...
  ck "[$SH] model(reasoning)"    "command-code:xiaomi/mimo-v2.5-pro" "$(model_of "$SH" 'debug this failure')"
  ck "[$SH] model(validation)"   "command-code:google/gemini-3.7-flash" "$(model_of "$SH" 'audit the code')"
  ck "[$SH] model(fast)"         "command-code:poolside/laguna-s-2.1-free" "$(model_of "$SH" 'fix typo in header')"
