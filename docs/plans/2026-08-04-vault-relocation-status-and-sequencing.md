...
2. ~~Re-confirm `thoughtseed-labs`'s registry-host baseline.~~ **Re-confirmed 2026-08-05** — status digest still byte-identical (122 files, same content) to the approved baseline, so it would still pass `assertRegistryHostClean()`. But HEAD moved (`ad855051…` → `90ffc943…`) between the original approval and this check — the repo is actively receiving commits in the background, not sitting static. Re-confirm again immediately before any real apply; do not treat this as settled indefinitely. Full detail in §2.
