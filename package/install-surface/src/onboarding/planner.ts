...
function remediationFor(probe: CapabilityProbe): string[] {
  const messages: Record<CapabilityProbe["reason_code"], string> = {
    ...
    ROUTING_ALIAS_AMBIGUOUS: "Keep exactly one mapping for the required semantic routing alias.",
    ROUTING_COMBO_MISSING: "Create or select the mapped combo in 9Router, then probe again.",
    ROUTING_COMBO_AMBIGUOUS: "Resolve duplicate combo names in 9Router before enabling this module.",
    ROUTING_COMBO_EMPTY: "Seat at least one current provider model in the mapped combo.",
    ROUTING_COMBO_CHANGED: "The combo changed during inspection; review its mapping and probe again.",
    ROUTING_MODEL_UNAVAILABLE: "Reconnect the provider or replace unavailable combo members using the live model catalog.",
    ROUTING_MODEL_NESTED: "Replace nested combo members with current provider models.",
    ROUTING_API_UNAVAILABLE: "Start the configured local 9Router service and rerun onboarding.",
    ROUTING_AUTH_UNAVAILABLE: "Repair 9Router management authentication and its owner-only metadata, then probe again.",
    ROUTING_RESPONSE_INVALID: "Inspect 9Router catalog consistency and version compatibility, then probe again.",
