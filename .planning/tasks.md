...
- [x] **T06** Phase 2 execute: converge public source; zero credentials/private db/logs/atlasRecall (te-dispatch-paid)
  ...
  - Done 2026-08-22: Plan 02-01 all six tasks green — Class A relocation (59 files → `~/.temperance_engine/private-overlay/`, overlay pre-existing content untouched), ISA.md redacted (2182 ISC baseline held), 16 historical docs labeled+redacted, guard upgraded to role taxonomy + node_modules pruning + root-level fail-closed sweep (canary cycle proves planted violation fails), NEXT-WAVE `cwd` dropped (watcher.ts fallback evidence), react-bits-pro gitignored, README genericized. `bash ./verify.sh` exit 0; widened scan zero unexplained hits; credential sweep clean. Later verifier hardening superseded whole-file guard-spec exclusions with line-scoped grammar handling; the fixture namespace is reserved for synthetic paths; ajv dep install needed for green tests (env, not code). Plan 02-02 complete: `CANDIDATE-INVENTORY.md` authored — 16-row six-column inventory, all 11 `package/*` + 4 `skills/*` dirs mapped exactly once via fragment record ids or repo-native routes, relocation tooling recorded once as NEVER-SHIP; coverage/fragment-citation/privacy verify loops exit clean. Phase 2 success criteria 1–3 all evidenced.
...
- [x] **T08** Phase 3: profiles minimal/full/no-voice + transactional install/update/rollback/uninstall (te-dispatch-paid)
  - AC: Phase 3 success criteria demonstrable in tests; failed apply never reads as committed.
  - Done 2026-08-23: All four plans executed and merged sequentially via te-dispatch-paid worktree agents. **Plan 03-01** (Doctor v2): five-section registry, v2 schema with inventory_digest, M1–M6 consumer migration map — merged at `5a7f8aa`. **Plan 03-02** (Profiles): `ProfileId` type, schema enum constraint, compile-time filter, all 18 records re-tagged (`minimal|full|no-voice`), `compile --profile` CLI preview — merged at `e249b43`. **Plan 03-03a** (Safety foundation): `Journal` class (fsync-before-return, crash recovery, retention pruning), hazard preflight (symlink/hardlink/drift/TOCTOU/dependency checks), `LifecycleIO` injection seam — merged at `96afbc4`. **Plan 03-03b** (Consumer layer): `planner.ts` (topological sort, NEVER-SHIP rejection, outcome taxonomy), `executor.ts` (staged atomic promotion, preimage backup), `receipts.ts` (redaction-safe, digest-linked), CLI verbs (`install|update|uninstall|rollback|receipt` with `--dry-run`, `--profile`, `--force`, `--select`, `--json`), deprecation notices on shell scripts — merged at `9dea961`. **Verification**: 130 tests green across 11 files (251 expect() calls). All Phase 3 success criteria evidenced: profile-resolved CompileResult feeds doctor + lifecycle; journal-before-mutation provable; crash recovery byte-exact; drift refusal blocks ambiguous destruction; outside-block content preserved; failed apply ≠ committed; receipts symbolic-clean; digest linkage demonstrated.
...
- [ ] **T17** Spec §3.4 host-side half of doctor M3: slim `~/.temperance_engine/router/temperance-surface-doctor.mjs` to private probes only + schema-conforming emitter validating against `doctor-report.v2.schema.json`; conformance test runs where `~/.temperance_engine` exists (te-build, host-side, after 03-01 lands the public v2 schema)
  - Origin: checker M7 on Plan 03-01 — the private half of M3 has a spec owner but no wave; recorded here so the deferral is deliberate.
  - AC: slimmed host file emits a report that validates against the published v2 schema; private probes absent from all public sections (proven by Plan 03-01's grep gates).

## Handoff

- SWARM (te-swarm-s): T01–T03, T12, T15
- PAID (te-dispatch-paid): T06, T08 (done — all four plans merged: 03-01 `5a7f8aa`, 03-02 `e249b43`, 03-03a `96afbc4`, 03-03b `9dea961`; 130 tests green), T09, T11
- BUILD (te-build): T17 — host-side §3.4, after 03-01 lands
