...
- [ ] **T08** Phase 3: profiles minimal/full/no-voice + transactional install/update/rollback/uninstall (te-dispatch-paid)
  ...
  - Discuss done 2026-08-22 (`08-DISCUSSION-LOG.md`, `08-CONTEXT.md`): four decisions ratified — profiles as eligibility enum on fragment records (compiler filters; no new config files), executor as `package/install-surface/src/lifecycle/{planner,journal,executor,receipts,hazards}.ts` with thin shell wrappers, transactions persisted at `~/.temperance_engine/transactions/<txid>/` (journal + preimage + receipt + before/after digests; rollback --select <txid>), T07 M1–M2 folded as wave 1 (Plan 03-01 doctor v2 + manifest section parity) before lifecycle. Plan 03-01 authored + checker iteration 1 applied (1 blocker / 7 major / 2 minor fixed). Plans split per checker M-5 on the original combined draft: **Plan 03-02 = profiles wave** (fragment re-tagging of all 18 `"default"` records + compile filter + CLI preview; checker fixes B-1/M-1/M-3/m-3 applied). **Plan 03-03 further split** per checker scope-sanity (7 tasks / 9 files exceeded threshold): **03-03a = safety foundation** (journal + hazards + dependency preflight; 4 tasks / 3 files) → **03-03b = consumer layer** (planner + executor + receipts + CLI verbs; 4 tasks / 5 files). All phantom `compileInventory()` references corrected to `compileFragments()`. All four plans checker-verified. Dispatch order: 03-01 → 03-02 → 03-03a → 03-03b via te-dispatch-paid.
...
- PAID (te-dispatch-paid): T06, T08 (Plans 03-01, 03-02, 03-03a, 03-03b all checker-verified; dispatch order: 03-01 → 03-02 → 03-03a → 03-03b), T09, T11
