...
Existing install-surface fragments declare whole-tree COPY ownership for router and bridge, but the current lifecycle executor cannot yet prove that ownership: it stages a single `readFile`, computes COPY hashes without comparing them, and its rollback does not guard drift or remove newly created paths. RO-00 is therefore a hard prerequisite for any temporary installation proof. It must add a reviewed directory-COPY primitive or an explicitly compiled per-file representation, declared-hash comparison, journaled absent/preimage/current state, drift-safe rollback, and focused temporary-root lifecycle tests. Semantic install lock changes are needed only if records change. Source-hash inventory and compiled manifest digest are separate: a compiled semantic lock digest alone is not content dependency closure.
...
After RO-00, build a test-owned temporary root with explicit state/home/config paths supplied to installer adapters; do not repurpose the shell's HOME/CODEX_HOME or use default global destinations. Stub or inject service/network/process integrations. If an installer cannot target the temporary root without touching global state, hold and refactor its test seam before the smoke. Use the real, now-proven COPY/lifecycle logic with a minimal reviewed fixture manifest, not ad hoc copying as the only installation proof.
...
| G2 temporary installation | G1 artifacts plus RO-00 lifecycle proof | Real installer-adapter dry-run/apply in temporary root, installed import/replay, corruption failures and rollback proof. Hold G2 while the file-only executor cannot prove the fragment's tree behavior. |
